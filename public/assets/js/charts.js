/**
 * Chart.js: demo data from window.mockData for legacy demo widgets (KPI IDs, charts).
 */
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (elTotalSales) {
        elTotalSales.innerText = `$${totalSales.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
    }
    if (elTotalPurchases) {
        elTotalPurchases.innerText = `$${totalPurchases.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
    }

    const elActiveClients = document.getElementById('kpi-active-clients');

    if (elActiveClients) {
        elActiveClients.innerText = activeClients;
    }

    const elActiveSuppliers = document.getElementById('kpi-active-suppliers');

    if (elActiveSuppliers) {
        elActiveSuppliers.innerText = activeSuppliers;
    }
